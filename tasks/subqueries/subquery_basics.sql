...
USE SAKILA;
-- 10 subqueries užduočių iš Sakila duomenų bazės su sprendimais, apimančių įvairius subquery tipus: single-row, multi-row, correlated, nested, scalar
...
SELECT 
    ...
    (SELECT 
        ...
        FROM
            rental r
        WHERE
            r.customer_id = c.customer_id) AS nuomos
FROM
    customer c;
